...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/04/09 20:24:32 by mde-arpe          #+#    #+#             */
/*   Updated: 2022/04/10 06:43:03 by mde-arpe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_next_size(int fd, t_read *red)
{
	...
	red->already_read = ft_calloc(BUFFER_SIZE + 1, 1);
	if (!(red->already_read))
		return (-1);
...
{
	static t_l_read	*first_ele;
	t_l_read		*current;
	char			*ret;
	int				ret_counter;
	int				n_w;

	current = find_or_add(&first_ele, fd);
	if (!checks_and_assignation(&(current->content), &ret, fd))
		return (free_list_elem(&current, &first_ele), NULL);
	ret_counter = 0;
	n_w = norminette_while(&(current->content), &ret, &ret_counter, fd);
	if (!n_w)
		return (free_list_elem(&current, &first_ele), NULL);
	if (n_w == 1)
		return (ret);
	ret[ret_counter] = '\n';
	if (++(current->content.counter) == current->content.n_of_chars)
	{
		free_and_null(&(current->content));
		free_list_elem(&current, &first_ele);
	}
	return (ret);
}
